  ...
  outputs = { self, nixpkgs, flake-utils }:
    {
      lib.mkInstaller = pkgs: hookTypes:
        ...
        pkgs.writeShellScriptBin "install-git-hooks"
          ''
            ...
            ${pkgs.lib.concatStringsSep "\n" (nixpkgs.lib.mapAttrsToList (type: hooks: installHookScript (mkHook type hooks)) hookTypes )}
      ...
      lib.mkUninstaller = pkgs: pkgs.writeShellScriptBin "uninstall-git-hooks"
        ''
        ...
        devShell =
          let
            nixFormatHook = pkgs.writeShellScriptBin "check-rust-format-hook" ''
              ...
              RESULT=$?
              [ $RESULT != 0 ] && echo "Please run \`nixfmt\` before committing"
              exit $RESULT
            '';

            hookInstaller = pkgs.git-hook-installer { pre-commit = [ nixFormatHook ]; };
